...
// new_note: the order of old node affecting to the way of expanding chain.
void expanding_chain(Embedding* embedding, Graph* P, Pass* ps) {
    ...
    vector<Point*> *free_neighbors = new vector<Point*>[200];
...
//                check_f[i*topo_row*8+j*8+k] = check_embedded;
    ...
    if (good_pos->x == -1) {
        ...
        bool check_first = true, check_alone = false;
        int curr_depth = 0;
        int old_size = embedding->get_selected()->size();
        allocate_node(root, new_color, embedding, check_first, check_alone, curr_depth, P);
        //cout<<"curr_size: "<<embedding->get_selected()->size()-old_size<<endl;
        delete_tree(root);
        delete good_pos;
        delete[] label_HW;
        delete[] f;
                    ...
                    if (P->has_edge(prev_co, next_co) || (prev_co == next_co && prev_co != -1)) {
                        if (find(old_node->begin(), old_node->end(), prev_co) != old_node->end()) {
                            ...
                            if (find(old_node->begin(), old_node->end(), next_co) != old_node->end()) {
                                ...
                                if (P->get_degree(next_co) == 0) {
                                    co = prev_co;
                                } else {
                                    co = next_co;
                                }
                                /* new_change
                                if (rand()%(P->get_degree(prev_co) + P->get_degree(next_co)) < P->get_degree(prev_co)) {
                                    co = prev_co;
                                } else {
                                    co = next_co;
                                }
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < m; ++i)
    {
        int temp1 = atoi(argv[curr_idx++]);
        int temp2 = atoi(argv[curr_idx++]);
        P->add_edge(temp1, temp2);
        //cout <<temp1<<' '<<temp2<<endl;
    }
    
    
    int topo_row = atoi(argv[curr_idx++]);
    int topo_column = atoi(argv[curr_idx++]);
    int seed_limit = atoi(argv[curr_idx++]);
    int is_beginning = atoi(argv[curr_idx++]);
    ...
    if (is_beginning == 0) {
        ...
        //cout<<seed_set->at(0) << ' ' <<seed_set->at(1)<< ' ' <<seed_set->at(2)<< ' ' <<seed_set->at(3)<< ' ' <<seed_set->at(4) <<endl;
        initialize_embedding(embedding, seed_set, seed_limit);
    } else {
        int curr_node = atoi(argv[curr_idx++]);
        vector<int>* old_node = new vector<int>();
        int old_len = atoi(argv[curr_idx++]);
        for (int i = 0; i < old_len; ++i) {
            old_node->push_back(atoi(argv[curr_idx++]));
        }
        Pass* it = new Pass(curr_node, old_node);
        
        int embedding_len = atoi(argv[curr_idx++]);
        for (int i = 0; i < embedding_len; ++i) {
            int x = atoi(argv[curr_idx++]);
            int y = atoi(argv[curr_idx++]);
            int z = atoi(argv[curr_idx++]);
            int color = atoi(argv[curr_idx++]);
            embedding->append(x, y, z, color);
        }
        //embedding->print();
    ...
    fo << max_row+2 <<' '<<max_col+2 << endl;
    fo << check_expand << endl;
    return 0;
    //embedding->print();
}
